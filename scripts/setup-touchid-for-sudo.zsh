...
__config_file_contents="# This file was generated by the bootstrap script at:
...
__check_sudo_touchid_configured() {
  if ! [[ -r "${__config_file}" ]] || ! sudo grep -q "${__config_line}" "${__config_file}"; then
    # The file doesn't exist or the expected line is not present
    return 1
  fi

  # Check whether the expected line exists but is commented out...
  warn_msg="The expected config line in ${__config_file} seemed to be commented out!\nPlease uncomment the '${__config_line}' line or refer to ${__SE_ref} for more detailed instructions."
  escaped_config_line=$(printf '%q' "${__config_line}")
  if sudo grep -qE "^#[[:space:]]*${escaped_config_line}" "${__config_file}"; then
